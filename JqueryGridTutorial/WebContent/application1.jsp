...
<%
	...
	String sqlPgintn = "SELECT SQL_CALC_FOUND_ROWS * FROM registerdata limit "
			+ iPagNo + "," + iSwRws + "";
...
while(rsPgin.next()){
%>
<tr><td><%=rsPgin.getString(2)%></td>
<td><%=rsPgin.getString(1)%></td>
<td><%=rsPgin.getString(3)%></td>

<td><input type="button" name="edit" value="Edit" style="background-color:#49743D;font-weight:bold;color:#ffffff;" onclick="editRecord(<%=rsPgin.getString(1)%>);" ></td>
<td><input type="button" name="delete" value="Delete" style="background-color:#ff0000;font-weight:bold;color:#ffffff;" onclick="deleteRecord(<%=rsPgin.getString(1)%>);" ></td>
</tr> 
<%
}
%>
				...
				<td colspan="3">
					<div>
						<%
							...
							int cPge = 0;
							if (iTotRslts != 0) {
								cPge = ((int) (Math.ceil((double) iEnRsNo
										/ (iTotSrhRcrds * iSwRws))));
								int prePageNo = (cPge * iTotSrhRcrds)
										- ((iTotSrhRcrds - 1) + iTotSrhRcrds);
								if ((cPge * iTotSrhRcrds) - (iTotSrhRcrds) > 0) {
						%>
						<a href="application1.jsp?iPagNo=<%=prePageNo%>&cPagNo=<%=prePageNo%>"><<
							Previous</a>
						<%
							}
